...
window.addEventListener("load", changeScreenSize);
...
function scrolled() {
  ...
  document.getElementById("top").style.height =
    (difference - 14).toString() + "px";
  document.getElementById("top").style.marginTop =
    (360 - difference).toString() + "px";
  if (top >= 300) {
  ...
  document.getElementById("header").style.background =
    "rgba(20,32,57," + (top / 300).toString() + ")";
  document.getElementById("mobileMenuContainer").style.height =
    (height - 14).toString() + "px";

  document.body.style.backgroundImage =
    "linear-gradient(to bottom,white,rgba(20,32,57," +
    (top / height).toString() +
    "))";
}
...
function initialize() {
  ...
  const headers = document.getElementsByClassName("link");
  const headerMenus = document
    .getElementById("headerMenus")
  ...
  const mobileMenuOpeners = document.getElementsByClassName(
    "mobileDropDownOpener"
  );
  const mobileMenuDropDowns = document.getElementsByClassName("mobileDropDown");
