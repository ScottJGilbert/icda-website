...
function changeScreenSize() {
    ...
    document.getElementById("mobileMenu").style.height = (document.documentElement.clientHeight - 14).toString() + "px"
        ...
        headers[i].onmouseover = function () {
            try {
                ...
                if (document.documentElement.scrollTop <= 200) {
                    document.getElementById("headerMenus").style.transform = "translateY(-200px)"
                    headers[i].style.borderRadius = "0 0 30px 30px"
                }
                else {
                    document.getElementById("headerMenus").style.transform = "translateY(0)"
...
function scrollDown() {
    ...
    document.getElementById("topImages").getElementsByTagName("svg")[0].style.animationDuration = "0.5s"
    document.getElementById("topImages").getElementsByTagName("svg")[0].style.animationIterationCount = "1"
    setTimeout(function () {
        document.getElementById("topImages").getElementsByTagName("svg")[0].style.animation = "arrowStatic"
        document.getElementById("topImages").getElementsByTagName("svg")[0].style.animationDuration = "3s"
        document.getElementById("topImages").getElementsByTagName("svg")[0].style.animationIterationCount = "infinite"
    }, 3000)
...
function initializeCarousel() {
    let slides = document.getElementById("slideshow").getElementsByClassName("slide")
    slides[0].id = "middle"
}
...
function changeScreenSizeMain(width) {
    ...
    if (width < 1200) {
        document.getElementById("message").getElementsByTagName("section")[0].style.flexDirection = "column"
    }
    else {
        document.getElementById("message").getElementsByTagName("section")[0].style.flexDirection = "row"
    }
}

function changePageHeader(width) {
    if (width <= 1400) {
        document.getElementById("middle").getElementsByTagName("p")[0].style.width = (width - 60).toString() + "px"
        document.getElementById("carouselAdditional").style.width = (width - 60).toString() + "px"
    }
    else {
        let margins = (width - 1400)/2
        document.getElementById("middle").getElementsByTagName("p")[0].style.width = 1340 + "px"
